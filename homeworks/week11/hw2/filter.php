<?php
  ...
  */
  $current_page = 1;
  $per_page = 9;
  $sql = "SELECT COUNT(posts.id) AS total FROM posts WHERE is_deleted=0 AND category_id=?";
  ...
  <main class="main">
    <div class="container">
      ...
      <?php
        ...
        ';
        $sql = "SELECT * FROM posts WHERE is_deleted=0 AND category_id=? ORDER BY created_at DESC LIMIT ? OFFSET ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param('sii', $category_id ,$per_page, $offset);
        $stmt->execute();
        $result = $stmt->get_result();
        ...
        <div class="d-flex justify-content-center">
          <ul class="pagination">
            <li class="page-item <?php echo $current_page === 1 ? 'disabled' : '' ?>">
              <a class="page-link" href="filter.php?category_id=%s&page=<?php echo $current_page - 1?>">Previous</a>
            </li>
            <?php
              $template = '
              <li class="page-item %s">
                <a class="page-link" href="filter.php?category_id=%s&page=%d">%d</a>
              </li>';
            ...
            <li class="page-item <?php echo $current_page === $total_page ? 'disabled' : '' ?>">
              <a class="page-link" href="filter.php?category_id=%s&page=<?php echo $current_page + 1?>">Next</a>
            </li>
          </ul>
        </div>
      </nav>
